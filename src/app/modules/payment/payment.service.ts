...
const getAllPaymentList = async (query : Record<string , unknown >) => {
  ...
  const skip = page - 1
  const paymentList = await PaymentModel.find().skip(skip).limit(limit)
  return paymentList
}


const getUserPaymentList = async (userId:string,query : Record<string , unknown >) => {
  
  const page = query?.page ? Number(query.page) : 1 ;
  const limit = query?.limit ? Number(query.limit) : 10
  const skip = page - 1
  const singlePayment = await PaymentModel.find({userId}).skip(skip).limit(limit)

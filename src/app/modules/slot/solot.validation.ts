...
export const slotValidationSchema = z.object({
  body: z.object({
    ...
    // Handle errors
    if (!isValidStartMinutes || !isValidEndMinutes || !isValidDuration || !isValidEndTime || !isWithinAllowedRange) {
      return false;
    }

    return true;
  }, {
    message: 'Start time and end time must both be on the hour, end time must be at least 1 hour after start time, and end time must be within the allowed range (e.g., not exceeding 20:00).',
    path: ['endTime'],
  }),
});
